    ...
    *,
    ...
    return execute_read_query(
        ...
        WHERE v.status IN ['scheduled', 'in_transit']
          AND toFloat(coalesce(v.remaining_capacity_ton, 0)) > 0
          AND toFloat(coalesce(s.avg_monthly_volume_ton, 0)) > 0
          AND ($commodity IS NULL OR toLower(c.name) CONTAINS toLower($commodity))
        WITH v, p, s, c,
             CASE
               WHEN toFloat(s.avg_monthly_volume_ton) > toFloat(v.remaining_capacity_ton)
               THEN toFloat(v.remaining_capacity_ton)
               ELSE toFloat(s.avg_monthly_volume_ton)
             END AS offered
        RETURN s.id AS supplier_id, s.business_name AS supplier_name,
               c.id AS commodity_id, c.name AS commodity_name,
               p.id AS port_id, p.name AS port_name, v.id AS voyage_id,
               offered AS available_weight_ton,
               toFloat(v.remaining_capacity_ton) AS remaining_capacity_ton,
               s.rating AS rating, s.verified AS verified,
               offered > 0 AND offered <= toFloat(v.remaining_capacity_ton) AS capacity_compatible,
